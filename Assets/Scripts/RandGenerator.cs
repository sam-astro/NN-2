...
namespace NeuralNetConsoleCS
{
    public class RandGenerator
    {
        ...
        public RandGenerator(double frequency,
        ...
        private void GenerateData()
        {
            ...
            for (uint index = 0; index < bufferSize - 1; index++)
            {
                _dataBuffer[index] = Convert.ToInt16(amplitude * (((double)index + 1) / (bufferSize - 1)*2.0d-1.0d));
            }
        }

        public double[] NoiseMaker(double[] input)
        {
            Random random = new Random();
            for (int i = 0; i < input.Length; i++)
            {
                input[i] *= 0.9d+(random.NextDouble()-0.5d)/10.0d;
            }
            return input;
        }
    }
